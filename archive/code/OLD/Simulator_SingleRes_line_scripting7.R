...
process_param <- data.frame(
  c(getParmsLognormForMoments(mean = 80, var = 5)),
  c(getParmsLognormForMoments(mean = 90, var = 5)),
  c(getParmsLognormForMoments(mean = 95, var = 5)),
  c(getParmsLognormForMoments(mean = 100, var = 5)),
  c(getParmsLognormForMoments(mean = 105, var = 5)),
  c(getParmsLognormForMoments(mean = 115, var = 5)),
  row.names = c("mean","sd")
)
...
builder_env <- function(){
  traj_complete <- join(builder_first_act(),
                        ...
                        builder_middle_act(num = 3, delay_ind = 1),
                        ...
                        builder_middle_act(num = 6, delay_ind = 1),
                        builder_last_act())
...
cat("Raccolta in unico dataframe\n")
...
##### Costruzione finestre per calcolo utilizzo #####
...
##### Monitoraggio code #####
data_queue_monitoring_list <- list()
for(i in 1:tot_replications){
  data_queue_monitoring_list[[i]] <- get_mon_resources(env_replication[[i]]) %>%
...
data_queue_monitoring <- bind_rows(data_queue_monitoring_list) %>%
  group_by(resource) %>%
  summarise(queue = mean(queue),
            time = mean(time))


# waiting
# wait_plot <- plot(get_mon_arrivals(env_replication[[1]]), 
#                   metric = "waiting_time")

##### Stampa risultati #####
print(data_utilization %>% select(-Timestamp_res))
print(data_queue_monitoring %>% select(-time))
