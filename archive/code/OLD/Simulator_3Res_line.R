...
act_builder_singleRes <- function(env_name = Env_5, num = 1, delay_ind = 1){
  ...
  AttivitaX <- trajectory(trj_name) %>%
    ...
    release(res_name) %>%
    send(signal_out) # Forse posizione sbagliata
  
  return(AttivitaX)
}

Attivita2 <- act_builder_singleRes(num = 2, delay_ind = 2)
Attivita3 <- act_builder_singleRes(num = 3, delay_ind = 3)
Attivita4 <- act_builder_singleRes(num = 4, delay_ind = 4)
Attivita5 <- act_builder_singleRes(num = 5, delay_ind = 3)
Attivita6 <- act_builder_singleRes(num = 6, delay_ind = 2)
...
Env_5 <- 
  ...
  add_generator(name_prefix = "",
                ...
                mon = 2,
                distribution = function() {c(rep_len(1,15000),-1)})
