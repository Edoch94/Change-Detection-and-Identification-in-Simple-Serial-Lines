...
data_sim_in <- data_sim_in_NOTaggr %>%
  data_sim_in_list %>% 
  bind_rows() %>%
...
data_out <- data_sim_in %>%
  ...
  # TREND MODIFICATO (RAPPORTO INVECE DI DIFFERENZA)
...
# mutate(True_blocking = (Blocking - Starvation > 0) * (Blocking - Starvation)) %>%
# mutate(True_starvation = (Starvation - Blocking > 0) * (Starvation - Blocking)) %>%
# mutate(True_idle = True_blocking + True_starvation)
