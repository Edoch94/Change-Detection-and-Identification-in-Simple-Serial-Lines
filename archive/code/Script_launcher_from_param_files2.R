...
if(Sys.info()["user"] == "Edoardo pc"){
  plot_general_output_path <- file.path("D:","Politecnico","Plot_sim")
} 
...
process_param <- data.frame(
  ...
  c(getParmsLognormForMoments(mean = 16, var = std_value^2)), # 11
  c(getParmsLognormForMoments(mean = 17, var = std_value^2)), # 12
  row.names = c("mean","sd")
)
colnames(process_param) <- as.character(c(1:12))
