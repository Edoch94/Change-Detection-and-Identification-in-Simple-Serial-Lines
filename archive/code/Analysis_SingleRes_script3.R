...
data_out_standard <- data_out %>%
  ungroup() %>%
  select(CaseID, Activity, Time1, Time2, Time3, Waiting, Processing, Blocking, Diff1, Diff2, Diff3, Waiting_trend, Processing_trend, Blocking_trend) %>%
...
data_windows <- data_out_standard %>%
  ...
  mutate(Blocking_prob = Blocking_time_MEAN/lagged_Output_diff_MEAN) %>%
